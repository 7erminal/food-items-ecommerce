...
type Props = {
    items: Array<ItemFeature> | undefined
    categoryFilter: string
    updateCart: (item: Item, number_: number, action: string)=>void
    title: string
}

const ItemFeatures: React.FC<Props> = ({items, title, categoryFilter, updateCart})=>{
    // console.log("Items received is ")
    // console.log(items)
                                        ...
                                        
    {
        items != null || items != undefined ?
            items.length > 0 ?
            items.filter((it: ItemFeature)=>it.Item.Category.CategoryName==categoryFilter || categoryFilter == "all").map((it: ItemFeature, i: number)=>{
